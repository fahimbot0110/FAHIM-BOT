...
module.exports = {
  ...
  run: async function ({ api, event, args }) {
    if (args.length === 0) {
      return api.sendMessage("⚠️গান এর নাম লিখো আগে😒", event.threadID);
    }
    ...
    try {
      ...
      const apiUrl = `https://rahat-music-w8d1.onrender.com/download?url=${encodeURIComponent(videoUrl)}&type=${mediaType}`;
